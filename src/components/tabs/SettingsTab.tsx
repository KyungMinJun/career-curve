...
export function SettingsTab() {
  ...
  const appliedCount = jobPostings.filter(
    (j) => j.status !== "reviewing"
  ).length;
  ...
  const totalCredits = subscription
    ? subscription.aiCreditsRemaining + subscription.aiCreditsUsed
    : 0;
  const creditPercentage =
    totalCredits > 0
      ? ((subscription?.aiCreditsRemaining || 0) / totalCredits) * 100
      : 0;
  ...
  return (
    <div className="flex flex-col h-full items-center">
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-20 space-y-4 scrollbar-hide w-[90%] lg:w-[600px]">
        {/* Summary Card */}
        <div className="w-full bg-gradient-to-br from-primary/10 to-accent rounded-2xl p-5 border border-primary/10">
        ...
        {subscription && (
          <div className="bg-card rounded-xl border border-border p-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Badge
                variant={
                  subscription.planName === "free" ? "secondary" : "default"
                }
              >
